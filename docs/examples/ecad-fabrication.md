...
attribute (the net-compare datum, read straight from the Gerbers), and *every* Gerber gains a
...
their source pad — what an AOI / paste-inspection tool reads), and a silk refdes / value / courtyard
stroke the `%TO.C` of the component it marks — so the X2 object attributes are complete across every layer.
...
blank), the Gerber X2 `%TA` aperture functions on the mask / paste (the X2 object attributes — net,
`FileFunction`, `.C` / `.P` and `%TA` on the copper, `.C` / `.P` on the mask / paste, and `.C` on the
silk — are complete), and a Gerber IMPORT of a foreign board (this is export).
