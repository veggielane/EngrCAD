...
| `export` | Writes `.step` (exact B-Rep, one file per part), `.stl`/`.obj`/`.off` (meshes merged with instance transforms), `.3mf`/`.amf` (one named object per part), `.glb`/`.gltf` (the assembly hierarchy kept), `.vtu` (geometry plus every part's simulation results, the array count reported), or `.png`. |
