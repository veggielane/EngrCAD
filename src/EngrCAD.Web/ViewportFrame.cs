...
///
/// <para><b>Scope, stated rather than implied.</b> Section planes, selection highlight
/// and baked occlusion are later rungs; their shared rules already exist
...
public static class ViewportFrame
{
        ...
        double pixelScale = 1.0)
    {
        ...
        for (int i = 0; i < instances.Count; i++)
        {
            ...
            switch (modes[i])
            {
                case EffectiveMode.ShadedWithEdges when instance.EdgeKey is not null && instance.EdgeVertexCount > 0:
                    draws.Add(Line(instance.EdgeKey, ColumnMajor(instance.World), EdgeColor,
                ...
                case EffectiveMode.Wireframe when instance.WireKey is not null && instance.WireVertexCount > 0:
                    draws.Add(Line(instance.WireKey, ColumnMajor(instance.World), Rgb(instance.Color),
        ...
        AppendTranslucent(draws, instances, modes, eye);
    ...
    private static void AppendTranslucent(
        ...
        EffectiveMode[] modes, in Vector3d eye)
    {
        int count = 0;
        for (int i = 0; i < instances.Count; i++)
        {
            if (modes[i] == EffectiveMode.Translucent)
                count++;
        }
        if (count == 0)
            return;
        ...
        {
            var instance = instances[order[k]];
            draws.Add(new DrawCall
            {
                Program = MeshProgram,
                Geometry = instance.GeometryKey,
