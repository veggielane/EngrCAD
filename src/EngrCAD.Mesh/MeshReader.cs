...
/// (.obj), <see cref="OffReader"/> (.off), or <see cref="VrmlReader"/> (.wrl, VRML97).
/// See <see cref="MeshReadResult"/> for the contract: dirty files return the welded
/// soup + diagnostics instead of throwing.
/// </summary>
public static class MeshReader
{
    ...
    public static bool SupportsExtension(string extension)
    {
        ...
        return e is ".stl" or ".obj" or ".off" or ".wrl";
    }

    /// <param name="path">Mesh file to read (.stl, .obj, .off, or .wrl).</param>
    ...
    /// (binary STL is float32-quantized; VRML coordinates are read verbatim with no
    /// unit factor).</param>
    public static MeshReadResult ReadFile(string path, double weldTolerance = 1e-9) =>
        Normalize(Path.GetExtension(path)) switch
        {
            ...
            ".wrl" => VrmlReader.ReadFile(path, weldTolerance),
            var e => throw new NotSupportedException(
                $"Unsupported mesh format '{e}'. Supported extensions: .stl, .obj, .off, .wrl."),
        };
