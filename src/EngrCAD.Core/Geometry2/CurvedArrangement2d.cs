...
public sealed class CurvedArrangement2d
{
    ...
    /// departure directions, then the departure curvature.
    ///
    /// <para>The curvature rule follows from the local expansion p(s) = v + s·d + ½s²κ·n̂:
    /// two edges leaving along the same d separate at second order, and the one with the
    /// LARGER signed curvature sits further counter-clockwise. Equal in both means one
    /// carrier (see the class remarks), which the arrangement has already deduped — so
    /// reaching that state means two distinct carriers osculate, and the walk refuses rather
    ...
    private int CompareCcw(int v, int e1, int e2)
    {
        ...
        throw new InvalidOperationException(
            $"Curved arrangement cannot order two edges leaving {_vertices[v]}: they share a tangent "
            + "and a curvature but not a carrier (osculating curves are outside the line/arc tier).");
    }
