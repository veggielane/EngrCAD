...
{
    // 2^-53: half an ulp of 1.0 — the unit roundoff used in all of Shewchuk's bounds.
    private const double Epsilon = 1.1102230246251565e-16;
    // 2^27 + 1: splits a 53-bit significand into two 26-bit halves (Dekker splitting).
    ...
    // ---- expansion arithmetic building blocks (Shewchuk's macros) ----
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static void TwoDiffTail(double a, double b, double x, out double y)
    {
        double bvirt = a - x;
        double avirt = x + bvirt;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static void Square(double a, out double x, out double y)
    {
        x = a * a;
        Split(a, out double ahi, out double alo);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static void TwoOneSum(double a1, double a0, double b, out double x2, out double x1, out double x0)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static void TwoTwoSum(double a1, double a0, double b1, double b0,
    ...
    /// </summary>
    private static int FastExpansionSumZeroElim(
    ...
    /// </summary>
    private static int ScaleExpansionZeroElim(ReadOnlySpan<double> e, int elen, double b, Span<double> h)
    {
        Split(b, out double bhi, out double blo);
    ...
    private static double Estimate(ReadOnlySpan<double> e, int elen)
    {
        double q = e[0];
