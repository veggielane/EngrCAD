...
public static class Intersect3d
{
    ...
    public static bool RayTriangle(
        ...
        RayTriangleEdges(ray, a, b - a, c - a, out t);

    /// <summary>
    /// The same predicate over a triangle already stored as <b>corner + two edge
    /// vectors</b>. A caller that tests one triangle against many rays — the ambient
    /// occlusion bake widens its whole mesh once per bake, then fires 16 rays per vertex
    /// — pays for <c>b - a</c> and <c>c - a</c> once instead of per ray.
    /// <para>
    /// The corner-form overload above is one line of delegation to this, which is the
    /// point: two calling conventions, one algorithm, no possibility of drift.
    /// </para>
    /// </summary>
    /// <param name="ray">The ray; direction need not be unit length.</param>
    /// <param name="a">The corner the edges emanate from.</param>
    /// <param name="e1">First edge vector (<c>b - a</c>).</param>
    /// <param name="e2">Second edge vector (<c>c - a</c>).</param>
    /// <param name="t">Parameter along the ray direction of the hit; 0 on a miss.</param>
    /// <returns>True when the ray's line crosses the triangle's interior or boundary.</returns>
    public static bool RayTriangleEdges(
