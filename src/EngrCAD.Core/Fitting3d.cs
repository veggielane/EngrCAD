...
public static class Fitting3d
{
    ...
    /// tightest box WITH those axes). PCA axes are a good-fit heuristic that needs no hull
    /// and handles degenerate clouds — flat or collinear sets just get zero extents along
    /// the empty directions; for the true minimum-volume box use
    /// <see cref="MinVolumeBox"/>, which needs the cloud's convex hull. Throws on an empty
    /// input.
    /// </summary>
