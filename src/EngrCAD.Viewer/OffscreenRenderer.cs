...
/// because both passes draw with the same shaders, camera math, and furniture geometry
/// from <c>RenderCore.cs</c> (<see cref="ViewerShaders"/>/<see cref="CameraMath"/>/
/// <see cref="RenderGeometry"/>). The viewport-only features of the shared mesh shader
/// are disabled here by their neutral uniforms (uHighlight 0, uSectionEnabled 0,
/// uAlpha 1).
/// </summary>
public static class OffscreenRenderer
{
        ...
        GL gl, IReadOnlyList<PartInstance> instances, int width, int height, CameraState? camera, bool furniture)
    {
        ...
        gl.Uniform1(uLineSectionEnabled, 0f);   // no section plane offscreen
        ...
        // Neutral uniforms for the viewport-only shader features.
        gl.Uniform1(gl.GetUniformLocation(meshProgram, "uHighlight"), 0f);
        ...
        {
            CameraMath.WriteColumnMajor(model, matrix);
