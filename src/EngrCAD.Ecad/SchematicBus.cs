...
/// wire with diagonal ENTRY stubs ripping members off it, and a bus-VECTOR label
/// <c>NAME[first..last]</c> (KiCad's notation, member <c>NAME</c>+i — so <c>DATA[0..3]</c> is
/// DATA0..DATA3, and a reversed <c>DATA[3..0]</c> the same four in the drawn direction).
///
...
public sealed class SchematicBus
{
        ...
        SheetTextAnchor labelAnchor = SheetTextAnchor.Left)
    {
        ...
        Members = ExpandMembers(name, first, last);
    }

    /// <summary>The bus base name (<c>"DATA"</c>).</summary>
    public string Name { get; }

    /// <summary>The first vector index (inclusive).</summary>
    public int First { get; }

    /// <summary>The last vector index (inclusive).</summary>
    public int Last { get; }
    ...
    /// <summary>The expanded member net names (<c>NAME</c>+i), in the DRAWN direction — the same
    /// expansion the bus import reads, so a bus-vector label is never mistaken for a signal net.</summary>
    public IReadOnlyList<string> Members { get; }

    /// <summary>The bus-vector label text (<c>NAME[first..last]</c>).</summary>
    public string Label =>
