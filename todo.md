...
- [ ] **`?report`'s `annotationPixels` stopped measuring what its name says.** The
  ...
  likeliest suspect is the occlusion-aware annotation work, which gave the overlay two depth
  passes over one upload. The recorded figures are updated to what it measures today; what
  is wanted is either the cause or a classifier that counts only overlay-coloured pixels.
