...
- [ ] **Curved-2D-tier follow-ups** (the lines-and-arcs tier ✅ landed and is
  ...
  - [ ] **`ContainedIn` is O(cells × operand edges)** here as well. The POLYGONAL twin's
    verdict applies first and is retired with it: a full point-location index was built,
    measured at 1.0× on the filed workload and DECLINED, and the re-benchmark reads 89.8 /
    154.6 ms for 120 / 480 overlapping 32-gons — 4× the polygons for 1.72× the time, because
    a balanced fold keeps the CELL count tiny exactly where the operand vertex counts grow, so
    the product never gets large. Do not build the curved one without first measuring a
    consumer whose result KEEPS many cells against many-edge operands.
