...
4. **B-Rep engine** 🔶 modeling operations done — curves (`Line3d`, `Circle3d`, `NurbsCurve` with exact rational conics, `ReversedCurve`/`TransformedCurve` wrappers with `Underlying` for sampling rules), surfaces (`PlaneSurface`, `CylinderSurface`, `SphereSurface`, `NurbsSurface`, `ExtrudedSurface`, `RevolvedSurface`, `SweptSurface` with rotation-minimizing frames), topology (`BrepSolid`→…→`BrepVertex` with `Validate` + Euler–Poincaré), `SolidFactory`: `MakeBox`/`MakeCylinder` + **`Extrude` (incl. shear), `Revolve` (full turn), `Sweep` (RMF along open paths)** over `Profile` (planar closed chain; winding auto-corrected). `BRepTessellator` handles planar/cylinder/extruded/revolved/swept faces with edge-consistent grid sampling. Note: default `Curve3d.TangentAt` uses 2nd-order one-sided differences at domain ends — sweep frames are sensitive to start-tangent error. **Still to do: surface–surface intersection, trimmed faces (holes, partial revolve caps), booleans, filleting**
