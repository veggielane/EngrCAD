...
public class PdfFollowUpTests
{
    ...
    public void TheDraftingSymbolsWinAnsiRefusesSurviveUnderAnEmbeddedFont()
    {
        ...
        // Under the built-in Helvetica the symbols become the WORD forms ASME Y14.5
        // permits, because Helvetica has no glyph for any of them: a drawing that says
        // "DEEP 12" is readable where one that cannot be written at all is not. The
        // substitution is public and documented (PdfDrawing.DraftingWords), so it is not
        // the silent "?" the refusal exists to prevent — and note the diameter sign keeps
        // its own older stand-in, U+00D8.
        var plain = new PdfDrawing();
        plain.AddText(new Vector2d(0, 0), callout, 3.5);
        Assert.Equal("Ø4.5 DEEP 12 CBORE Ø8",
            Assert.Single(PdfReadback.Parse(plain.ToPdf()).Texts).Value);

        // ... and under an embedded font every symbol travels AS ITSELF: the word forms
        // are the fallback for a font that cannot spell them, never a downgrade of one
        // that can.
